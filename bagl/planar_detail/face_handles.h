// Copyright Aaron Windsor 2007
// Copyright 2024 Mikael Persson - Modernized to C++20

#ifndef BAGL_BAGL_PLANAR_DETAIL_FACE_HANDLES_H_
#define BAGL_BAGL_PLANAR_DETAIL_FACE_HANDLES_H_
...
struct lazy_list_node {
  ...
  explicit lazy_list_node(const DataType& data) : data_(data), reversed_(false), has_data_(true) {}

  lazy_list_node(ptr_t left_child, ptr_t right_child)
      : reversed_(false), has_data_(false), left_child_(left_child), right_child_(right_child) {}

  DataType data_;
  bool reversed_;
  bool has_data_;
  std::shared_ptr<lazy_list_node> left_child_;
  std::shared_ptr<lazy_list_node> right_child_;
};
...
struct edge_list_storage<no_embedding, Edge> {
  ...
  void push_back(Edge /*unused*/) {}
  void push_front(Edge /*unused*/) {}
  void reverse() {}
  void concat_front(edge_list_storage<no_embedding, Edge> /*unused*/) {}
  void concat_back(edge_list_storage<no_embedding, Edge> /*unused*/) {}
  template <typename OutputIterator>
  void get_list(OutputIterator /*unused*/) {}
};

template <typename Edge>
struct edge_list_storage<recursive_lazy_list, Edge> {
  using node_type = lazy_list_node<Edge>;
  ...
  void reverse() { value->reversed_ = !value->reversed_; }
 ...
 private:
  ...
  void get_list_helper(OutputIterator o_itr, type root, bool flipped = false) {
    ...
    if ((flipped && !root->reversed_) || (!flipped && root->reversed_)) {
      get_list_helper(o_itr, root->right_child_, true);
      get_list_helper(o_itr, root->left_child_, true);
    } else {
      get_list_helper(o_itr, root->left_child_, false);
      get_list_helper(o_itr, root->right_child_, false);
    }
  }
};
...
struct edge_list_storage<std_list, Edge> {
  ...
  void concat_front(edge_list_storage<std_list, Edge> other) { value.splice(value.begin(), other.value); }

  void concat_back(edge_list_storage<std_list, Edge> other) { value.splice(value.end(), other.value); }
...
struct face_handle_impl {
  ...
  using edge_list_storage_t = typename edge_list_storage<StoreEmbeddingPolicy, edge_t>::type;
...
template <typename Graph, typename StoreOldHandlesPolicy = store_old_handles,
          typename StoreEmbeddingPolicy = recursive_lazy_list>
class face_handle {
 public:
  using vertex_t = typename graph_traits<Graph>::vertex_descriptor;
  using edge_t = typename graph_traits<Graph>::edge_descriptor;
  using impl_t = face_handle_impl<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy>;
  using self_t = face_handle<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy>;

  explicit face_handle(vertex_t anchor = graph_traits<Graph>::null_vertex()) : pimpl_(new impl_t()) {
    pimpl_->anchor = anchor;
  }

  face_handle(vertex_t anchor, edge_t initial_edge, const Graph& g) : pimpl_(new impl_t()) {
    vertex_t s(source(initial_edge, g));
    vertex_t t(target(initial_edge, g));
    vertex_t other_vertex = s == anchor ? t : s;
    pimpl_->anchor = anchor;
    pimpl_->cached_first_edge = initial_edge;
    pimpl_->cached_second_edge = initial_edge;
    pimpl_->cached_first_vertex = other_vertex;
    pimpl_->cached_second_vertex = other_vertex;
    pimpl_->true_first_vertex = other_vertex;
    pimpl_->true_second_vertex = other_vertex;

    pimpl_->edge_list.push_back(initial_edge);
  ...
  void push_first(edge_t e, const Graph& g) {
    pimpl_->edge_list.push_front(e);
    pimpl_->cached_first_vertex = pimpl_->true_first_vertex = source(e, g) == pimpl_->anchor ? target(e, g) : source(e, g);
    pimpl_->cached_first_edge = e;
  }

  void push_second(edge_t e, const Graph& g) {
    pimpl_->edge_list.push_back(e);
    pimpl_->cached_second_vertex = pimpl_->true_second_vertex =
        source(e, g) == pimpl_->anchor ? target(e, g) : source(e, g);
    pimpl_->cached_second_edge = e;
  }

  void store_old_face_handles() { store_old_face_handles_dispatch(StoreOldHandlesPolicy()); }

  vertex_t first_vertex() const { return pimpl_->cached_first_vertex; }

  vertex_t second_vertex() const { return pimpl_->cached_second_vertex; }

  vertex_t true_first_vertex() const { return pimpl_->true_first_vertex; }

  vertex_t true_second_vertex() const { return pimpl_->true_second_vertex; }

  vertex_t old_first_vertex() const { return pimpl_->old_handles.first_vertex; }

  vertex_t old_second_vertex() const { return pimpl_->old_handles.second_vertex; }

  edge_t old_first_edge() const { return pimpl_->old_handles.first_edge; }

  edge_t old_second_edge() const { return pimpl_->old_handles.second_edge; }

  edge_t first_edge() const { return pimpl_->cached_first_edge; }

  edge_t second_edge() const { return pimpl_->cached_second_edge; }

  vertex_t get_anchor() const { return pimpl_->anchor; }

  void glue_first_to_second(face_handle<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy>& bottom) {
    pimpl_->edge_list.concat_front(bottom.pimpl_->edge_list);
    pimpl_->true_first_vertex = bottom.pimpl_->true_first_vertex;
    pimpl_->cached_first_vertex = bottom.pimpl_->cached_first_vertex;
    pimpl_->cached_first_edge = bottom.pimpl_->cached_first_edge;
  }

  void glue_second_to_first(face_handle<Graph, StoreOldHandlesPolicy, StoreEmbeddingPolicy>& bottom) {
    pimpl_->edge_list.concat_back(bottom.pimpl_->edge_list);
    pimpl_->true_second_vertex = bottom.pimpl_->true_second_vertex;
    pimpl_->cached_second_vertex = bottom.pimpl_->cached_second_vertex;
    pimpl_->cached_second_edge = bottom.pimpl_->cached_second_edge;
  }

  void flip() {
    pimpl_->edge_list.reverse();
    std::swap(pimpl_->true_first_vertex, pimpl_->true_second_vertex);
    std::swap(pimpl_->cached_first_vertex, pimpl_->cached_second_vertex);
    std::swap(pimpl_->cached_first_edge, pimpl_->cached_second_edge);
  }

  template <typename OutputIterator>
  void get_list(OutputIterator o_itr) {
    pimpl_->edge_list.get_list(o_itr);
  }

  void reset_vertex_cache() {
    pimpl_->cached_first_vertex = pimpl_->true_first_vertex;
    pimpl_->cached_second_vertex = pimpl_->true_second_vertex;
  }

  void set_first_vertex(vertex_t v) { pimpl_->cached_first_vertex = v; }

  void set_second_vertex(vertex_t v) { pimpl_->cached_second_vertex = v; }

 private:
  void store_old_face_handles_dispatch(store_old_handles) {
    pimpl_->old_handles.first_vertex = pimpl_->true_first_vertex;
    pimpl_->old_handles.second_vertex = pimpl_->true_second_vertex;
    pimpl_->old_handles.first_edge = pimpl_->cached_first_edge;
    pimpl_->old_handles.second_edge = pimpl_->cached_second_edge;
  }
...
#endif  // BAGL_BAGL_PLANAR_DETAIL_FACE_HANDLES_H_
