...
template <typename G>
class adjlist_edges_from_out_edges : public std::ranges::view_interface<adjlist_edges_from_out_edges<G>> {
 public:
  using self_range = adjlist_edges_from_out_edges<G>;
  ...
  explicit adjlist_edges_from_out_edges(const G& g) : p_g(&g), v_range(vertices(*p_g)) {}
  explicit adjlist_edges_from_out_edges(const G&& g) = delete;
  ...
  struct iterator {
    ...
    iterator& operator--() {
      if (cur_oe.begin() == cur_oe.base_begin()) {
        // If cur_vi is begin, then this iterator is also begin, so decrement is undefined behavior.
        cur_oe = parent->get_fresh_eo_range(--cur_vi);
      } else {
    ...
    [[nodiscard]] bool operator==(const iterator& rhs) const {
      return (this->cur_vi == rhs.cur_vi) && ((this->cur_oe.empty() && rhs.cur_oe.empty()) || this->cur_oe.begin() == rhs.cur_oe.begin());
    }
    [[nodiscard]] bool operator!=(const iterator& rhs) const { return !(*this == rhs); }
    e_desc_t operator*() const {
      return *cur_oe.begin();
    }
    const e_desc_t* operator->() const { return &*(*this); }

    iterator(v_iter_t vi, const self_range* p) : cur_vi(vi), cur_oe(p->get_fresh_eo_range(vi)), parent(p) {}
  ...
  iterator begin() const { return {v_range.begin(), this}; }
  iterator end() const { return {v_range.end(), this}; }
