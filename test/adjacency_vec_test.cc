...
struct test_graphs {
  using VVL = adjacency_list<vec_s, vec_s, Kind, VertexProp, EdgeProp, GraphProp>;
  using VLL = adjacency_list<vec_s, list_s, Kind, VertexProp, EdgeProp, GraphProp>;
  using VPL = adjacency_list<vec_s, pool_s, Kind, VertexProp, EdgeProp, GraphProp>;

  using allgraphs = ::testing::Types<VVL, VLL, VPL>;
  using allgraphs_prop =
      ::testing::Types<std::tuple<VVL, PropMapMaps>, std::tuple<VLL, PropMapMaps>, std::tuple<VPL, PropMapMaps>>;
  using allgraphs_bund =
      ::testing::Types<std::tuple<VVL, BundleMaps>, std::tuple<VLL, BundleMaps>, std::tuple<VPL, BundleMaps>>;
};
